...
Does **not** auto-list until a real 402/paid flow runs. CDP indexes the settle payload, not verify: `paymentPayload.resource` must be an absolute `https://` URL and `extensions.bazaar` must be present (the Worker re-attaches both on settle if a client omits them). REST POST `/v1/research` advertises `type: "http"` (preferred by discovery search) and stays the catalog row agents already find for `MentionForge`. POST `/v1/research_mentions` is the same research call with `resource.url` ending in `research_mentions`, because a lone underscore query currently returns zero results catalog-wide and CDP then retries the query as a URL substring. MCP `research_mentions` still advertises `type: "mcp"` / `transport: "streamable-http"` on `${origin}/mcp`. Production seed: `PAY_ONCE=1 npm run seed-bazaar` (MCP, then REST `/v1/research`, then REST `/v1/research_mentions`, **$0.06**). Do not re-run `PAY_ONCE` for specialty tools. MCP clients send the payment payload object in `_meta["x402/payment"]`. See [go-live](go-live.md).
