...
Trial / sandbox: unwrapped tool (no x402 settle). Paid: `@x402/mcp` `createPaymentWrapper` — verify → research/persist (`executeUnpaidOrPreVerified`, which also bumps `/stats`) → settle → `onAfterSettlement` patches `tx_hash` and stores idempotency again. Bazaar `{ info, schema }` is attached so a successful CDP settle can index `research_mentions`. `resource.url` is `${origin}/mcp` (absolute HTTPS). `resource.description` is the short Bazaar search blurb (CDP `/verify` cap ~500 chars); `TOOL_DESC` remains the MCP tool description. MCP SDK v2 delivers request `_meta` on `ctx.mcpReq._meta`; the Worker also merges `ctx._meta` and `PAYMENT-SIGNATURE` / `X-PAYMENT` from the HTTP request onto the wrapper's `extra._meta`.
