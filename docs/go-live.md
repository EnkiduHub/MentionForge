...
1. After the keyword-discovery deploy: `PAY_ONCE=1 npm run seed-bazaar` (**$0.06**: MCP, REST `/v1/research`, REST `/v1/research_mentions`). CDP quality scores recompute on about a six-hour schedule, so search rank can lag the settle. Then query `https://api.cdp.coinbase.com/platform/v2/x402/discovery/search` (no API key) and confirm a `mentionforge.mentionforge.workers.dev` resource appears for `MentionForge`, `mentionforge`, `brand sentiment`, `social listening`, and `research_mentions`. Do not treat a pre-reseed result as proof. As of the 2026-09-22 catalog check, `MentionForge` already hit `/v1/research`, `social listening` was rank 7 of 9, `brand sentiment` was absent from 12 results, and unquoted `research_mentions` returned zero rows catalog-wide (quoted `"research_mentions"` hit the description). A lone underscore query falls back to a URL substring, which is why the alias path exists. Optional directory paste from `listings/` (mcp.so, Glama connector, Smithery URL). All must point at `https://mentionforge.mentionforge.workers.dev/mcp`, not a self-host command. PulseMCP waits on the Official Registry. Optional SIWX origin register is extra.
...
Keep the code MIT; the paid product is the production origin. Re-running `PAY_ONCE` spends **$0.06** (MCP + both REST catalog URLs) — do not repeat the seeds unless you intend to.
